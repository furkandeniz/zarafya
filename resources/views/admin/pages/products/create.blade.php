...
@section('content')
                        ...
                        <form id="productForm" action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data">
                            ...
                            <div class="form-group mb-4">
                                ...
                                <div class="border rounded p-3">
                                    ...
                                    <div class="form-text text-muted mt-1">JPG, JPEG, PNG veya WEBP. Maks. 2MB/fotoğraf. Birden fazla seçebilirsiniz.</div>
                                    @error('images.*')
    ...
    function handleImages(input) {
        Array.from(input.files).forEach(file => {
            ...
            addHiddenInput(input.closest('form'), file);
        });
        renderPreviews();
        input.value = '';
    }
    ...
    function rebuildVariants() {
        ...
        document.querySelectorAll('#variantTableBody tr').forEach(tr => {
            ...
            const stock = tr.querySelector('.v-stock')?.value ?? '';
            const price = tr.querySelector('.v-price')?.value ?? '';
            savedVariantData[key] = { stock, price };
        });
